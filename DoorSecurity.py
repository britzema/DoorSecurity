...
# Simulates a door security system, including an automated porch 
...
#
# Door lock status can be controlled via SignalMotor.py (can be
...
# Constants for mqtt
BROKER = 'YOUR BROKER'
MQTT_USERNAME = "YOUR USERNAME" # broker username
...
sent_to = [] # Can be any provider - gmail, aol, comcast, etc
...
pi = pigpio.pi()                # connect to pigpiod (stalizes the motor)
...
try:
    while True: # continuously run
        time.sleep(ITERATIONTIME) # to not waste cpu energy
...
# If you do want to turn the system off...
